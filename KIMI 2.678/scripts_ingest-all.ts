...
async function main() {
  ...
  for (const [name, result] of Object.entries(results)) {
    ...
    console.log(`${name.padEnd(20)} ${String(result.collected).padStart(4)} collected ${result.errors.length > 0 ? `| ${result.errors.length} errors` : ""}`);
  ...
  if (totalErrors > 0) console.log(`⚠️  ${totalErrors} errors (check above)`);
