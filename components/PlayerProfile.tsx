...
export function PlayerProfile({ employee }: { employee: Employee | EmployeeWithSurfaces }) {
  const data = Object.entries(employee.shotDistribution).map(([name, value]) => ({ name, value }));
  const pendingPosts = "postCount" in employee && employee.postCount === 0;
    ...
    <Card className="border-white/10 bg-white/[0.045]">
      ...
      <CardContent>
        {pendingPosts || !data.length ? (
          <div className="min-h-52 rounded-md border border-white/10 bg-black/20 p-4 text-sm leading-6 text-muted-foreground">
            Surface map is available, but post-level shot distribution is pending scrape/import data.
          </div>
        ) : (
          <>
            <div className="h-52">
